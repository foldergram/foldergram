...
describe('VideoProgressFooter', () => {
  it('maps a normal visual timeline from screen X to its playback position', async () => {
    const wrapper = mount(VideoProgressFooter, {
      props: {
        timeLabel: '0:00 / 2:00',
        seekOrientation: 'normal',
        currentTime: 0,
        duration: 120
      }
    });
    ...
    Object.assign(slider, {
      ...
      getBoundingClientRect: () => new DOMRect(100, 40, 300, 20)
    });
    ...
    expect(wrapper.emitted('seek-preview')?.[0]).toEqual([90]);
    expect(wrapper.emitted('seek')?.[0]).toEqual([90]);
  });
