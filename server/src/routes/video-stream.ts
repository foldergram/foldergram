...
export interface VideoStreamRouterOptions {
  /**
   * Extra gate applied before anything is streamed. Post shares use it so a token
   * only ever streams the post it was minted for; the library-wide router leaves it
   * undefined and relies on the API auth middleware alone.
   */
  authorizeImage?: (request: express.Request, imageId: number) => boolean;
  /**
   * How the master playlist addresses its media playlists. Share links live under a
   * token prefix, so they cannot point at `/api/videos/...`.
   */
  buildPlaylistPath?: (request: express.Request, imageId: number, quality: string) => string;
}
...
export function createVideoStreamRouter(options: VideoStreamRouterOptions = {}): express.Router {
  ...
  videoStreamRouter.get('/:id/hls/master.m3u8', async (request, response) => {
    ...
    sendPlaylist(
      ...
      buildMasterPlaylist(
        ...
        video.width,
        video.height,
        resolveOfferedQualities(video.width, video.height),
        buildPlaylistPath ? (imageId, quality) => buildPlaylistPath(request, imageId, quality) : undefined
      )
    );
  });
  ...
  return videoStreamRouter;
}

const videoStreamRouter = createVideoStreamRouter();
